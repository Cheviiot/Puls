...
остаётся `null`. `--ip` — отдельный однократный режим: пропускает выбор
сервера и все фазы измерения, работает только с `yandex` (для `speedtest` и
`all` возвращает usage error, код 2) и печатает собственный минимальный JSON
`{"external_ip": ...}`, не связанный со схемой `runResult`. Не добавляй
aliases и скрытые legacy flags без явной продуктовой причины.
